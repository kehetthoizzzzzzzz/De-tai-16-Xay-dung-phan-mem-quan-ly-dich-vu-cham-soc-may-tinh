...
class Ui_MainWindow(QtCore.QObject):
    ...
    def setupUi(self, MainWindow):
        ...
        self.cb_khach.setCompleter(completer_khach)

        # ==================================================
        # NHÂN VIÊN
        # ==================================================
        self.cb_nv = combo_box("Chọn nhân viên xử lý")
        ...
        self.cb_nv.setCompleter(completer_nv)

        # ==================================================
        # LOẠI MÁY
        # ==================================================
        self.type = combo_box("Chọn loại máy")
        self.type.addItems([
            "Laptop",
            "PC - Máy bàn",
            "Điện thoại",
        ...
        # LINH KIỆN
        # ==================================================
        self.cb_linhkien = combo_box("Chọn linh kiện")

        self.sl_linhkien = input_field("Số lượng")
        ...
        # TABLE LINH KIỆN
        # ==================================================
        self.table_lk = QtWidgets.QTableWidget(0, 4)
        ...
        # GIÁ TIỀN CÔNG
        # ==================================================
        self.gia = input_field("Nhập tiền công sửa chữa")
        self.gia.setValidator(
            QIntValidator(0, 1000000000)
        )

        # ==================================================
        # LABEL TỔNG LINH KIỆN
        # ==================================================
    ...
    def load_khach(self):
        ...
        query = """
        SELECT id_khach, ten, sdt
        FROM customers
        """

        for row in self.cursor.execute(query):
            text = f"{row[1]} - {row[2]}"
            self.cb_khach.addItem(text, row[0])
    ...
    def load_linhkien(self):
        ...
        for row in self.cursor.execute(query):
            ...
            self.cb_linhkien.addItem(text, row[0])
    ...
    def update_total(self):
        ...
        for row in range(self.table_lk.rowCount()):
            thanh_tien = int(
                self.table_lk.item(row, 3).text()
            )
            total += thanh_tien
    ...
    def create_ticket(self):
        ...
        # INSERT LINH KIỆN
        for row in range(self.table_lk.rowCount()):
            ten_lk = self.table_lk.item(row, 0).text()
            sl = int(self.table_lk.item(row, 1).text())
            thanh_tien = int(
                self.table_lk.item(row, 3).text()
            )
