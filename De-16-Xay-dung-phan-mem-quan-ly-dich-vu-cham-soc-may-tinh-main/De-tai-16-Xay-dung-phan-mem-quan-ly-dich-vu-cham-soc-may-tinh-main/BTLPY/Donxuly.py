...
class OrderUI(QtWidgets.QWidget):
    ...
    def show_detail(self, row, column):
        ...
        SELECT lk.ten,
               tlk.so_luong,
               tlk.gia
        FROM ticket_linhkien tlk
        JOIN linhkien lk
            ON tlk.id_lk = lk.id_lk
        WHERE tlk.id_ticket = ?
        """

        linhkiens = self.cursor.execute(
            lk_query,
            (id_ticket,)
        ).fetchall()
        ...
        if linhkiens:
            ...
            for lk in linhkiens:
                ...
                text_lk += (
                    f"• {ten_lk}\n"
                    f"   SL: {so_luong}\n"
                    f"   Thành tiền: {thanh_tien:,} đ\n\n"
                )

        else:

            text_lk = "Không có linh kiện"
...
💰 Tổng linh kiện: {tong_lk:,} đ

💵 Tổng hóa đơn: {result[8]:,} đ
