...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS ticket (
        ...
        FOREIGN KEY (id_khach)
            REFERENCES customers(id_khach)
            ON DELETE CASCADE,

        FOREIGN KEY (id_nv)
            REFERENCES staff(id_nv)
            ON DELETE SET NULL
    )
    """)

    # ========================
    # TABLE ticket_linhkien
    # ========================
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS ticket_linhkien (
    ...
    # SAMPLE LINHKIEN
    # ========================
    cursor.execute("SELECT COUNT(*) FROM linhkien")

    if cursor.fetchone()[0] == 0:

        cursor.executemany("""
        INSERT INTO linhkien (ten, loai, so_luong, gia)
        VALUES (?, ?, ?, ?)
        """, [
